...
export default class Database {
            ...
            if (!this.connection) {
                ...
                this.connection.query(sql, params, (err, result) => {
                    ...
                    for (let i = 0; i < result.length; i++) {
                        userList.push(
                            new User(result[i].username, result[i].password)
                        );
                    }
                    resolve(userList);
                });
    ...
    public insertImage(
        ...
        let sql =
            "INSERT INTO g_image(username, image_path, tags) VALUES(?, ?, ?)";
        ...
        return new Promise((resolve, reject) => {
            if (!this.connection) {
                ...
                this.connection.query(sql, params, (error, result) => {
                    ...
                    let tagList: string[] = [];
                    let splitedTag: string[] = result[0].tags.split(" ");
                    for (let tag of splitedTag) {
        ...
        return new Promise((resolve, reject) => {
            if (!this.connection) {
                ...
                this.connection.query(sql, (error, result) => {
                    ...
                    for (let i = 0; i < result.length; i++) {
                        imageList.push(
                            new Image(result[i].image_path, result[i].tags)
                        );
                    }
                    resolve(imageList);
                });
