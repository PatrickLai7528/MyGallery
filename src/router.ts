...
router.get("/login/:username/:password", (req: Request, res: Response) => {
    ...
    userService
        ...
        .catch(error => {
            res.end(
                JSON.stringify({ isValid: false, message: "Account not match" })
            );
        });
});
...
router.post("/signup/:username/:password", (req: Request, res: Response) => {
    ...
    userService
        ...
        .then((result: string) => {
            res.end(
                JSON.stringify({ isValid: true, message: "Sigup Success" })
            );
        })
        .catch(error => {
            console.log(error);
            res.end(
                JSON.stringify({ isValid: false, message: "Username repeated" })
            );
        });
});
...
router.get(
    ...
    BodyParser.json(),
    (req: Request, res: Response) => {
        let imagePath = "./upload_images/" + req.params.imagePath;
        imageService
            .getTagOf(imagePath)
            .then((tagList: string[]) => {
